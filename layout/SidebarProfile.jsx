          ...
          <ul className="space-y-3">
            {userDetailsSidebar?.basicInfo?.map((item) => (
              <li key={item?.id} className="flex text-sm">
                ...
                <span className="inline-block px-3.5 py-2 transition duration-300 border border-dashed text-black dark:text-white/70 border-platinum dark:border-greyBlack rounded-3xl md:px-5 md: md:py-2 hover:text-theme dark:hover:text-white">{item?.value}</span>
              </li>
            ))}
          </ul>
        </div>
